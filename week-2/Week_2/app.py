...
st.caption("TF-IDF + Logistic Regression • confidence estimate • explainable text signals")
st.divider()
...
if check_clicked:
    try:
        ...
        st.metric("Confidence estimate", f"{result['confidence']*100:.1f}%")
        st.progress(min(max(result["confidence"], 0.0), 1.0))

        st.subheader("Model details")
        st.write(f"**Raw ML prediction:** {result['raw_label']}")
        st.write(f"**ML confidence estimate:** {result['ml_confidence']*100:.1f}%")
...
st.divider()
st.caption(
    "⚠️ Academic project. The ML score is a statistical confidence estimate from a limited historical dataset. "
